# Hooks Reference — smith-ctx-claude

Detailed behavior for the guard/advisory hooks whose scripts live under
`smith-ctx-claude/scripts/`. See the repo root `README.md` "Hooks" section for
the cross-skill summary table, the full `settings.json` registration block,
and the manual verification checklist. `smith-git/references/HOOKS.md` and
`smith-standards/references/HOOKS.md` cover hooks owned by those skills.
...
| `attribution-model-stamp.sh` | PreToolUse (`Bash`) | Refreshes the model-id file `attribution.sh` reads for `Assisted-by:` trailers |
...
`smith-git/scripts/lib/git-command-tokenizer.mjs`, which layers `eval`/`sh -c`/`bash -c`
...
content. `smith-git/scripts/lib/git-command-tokenizer.mjs` is loaded via a `dynamic import()`
    ...
    "PreToolUse": [
      {
        ...
        "hooks": [
          { "type": "command", "command": "node \"$HOME/.claude/skills/smith-git/scripts/hooks/branch-guard.mjs\"" }
        ]
      },
      {
      ...
      },
      {
        ...
        "hooks": [
          ...
          { "type": "command", "command": "node \"$HOME/.claude/skills/smith-git/scripts/hooks/branch-name-guard.mjs\"" },
