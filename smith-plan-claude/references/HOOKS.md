...
Fires after ExitPlanMode tool is used. Creates `.pending-reload-<CWD_KEY>` flag with the most recently modified plan, enabling auto-reload after `/clear`. Output is minimal (non-actionable) to prevent Claude from following numbered "next steps" instead of exiting. Also creates an `.exit-marker` file that signals `enforce-clear.sh` to allow the stop, preventing the Stop hook from blocking the turn after ExitPlanMode.
