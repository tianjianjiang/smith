...
## CRITICAL: Pre-Send Gate (Primacy Zone)
...
1. **Draft, never direct-send** — use `slack_send_message_draft`, NEVER
   `slack_send_message`. NEVER draft or send a DM (company policy: all
...
4. **No formatting** — no bold, italic, backticks, or code blocks. Plain prose
   with bare identifiers and paths. (Links and mentions below are NOT formatting
   and ARE required.)
...
- `slack_send_message` (direct send) or any DM
- A body whose last line is not the attribution footnote
- Bare reference numbers without a URL; freehand PR titles
- Bold (`*x*`/`**x**`), italic (`_x_`), backticks, or code fences in the body
- Internal metadata / scratch reasoning in outbound text
...
- **Links**: Slack mrkdwn `<«url»|«display text»>` — never bare URLs, never
  markdown `[text](url)`.
- **Mentions**: `<@«USERID»>` (resolves + notifies). NEVER display names like
  `@佐藤` — they do not notify. Use ONE consistent form for all addressees
  (canonical `<@«USERID»> さん` where a project memory sets it).
