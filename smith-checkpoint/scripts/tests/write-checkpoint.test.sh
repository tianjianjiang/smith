...
grep -q 'settings.local.json' "$SHIM/stderr" && grep -q 'launcher' "$SHIM/stderr" && grep -q 'daemon' "$SHIM/stderr" && grep -q 'shell' "$SHIM/stderr" || fail "T1: stderr must name the four possible sources: $(cat "$SHIM/stderr")"
