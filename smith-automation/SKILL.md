...
- Use `Monitor` for event streams, `/loop` for paced polling, or just wait when the harness is already tracking the work — a Bash `sleep` loop to poll for state holds the turn open, blocks tool calls, burns context, and gives the model no chance to react between checks
- Let the harness re-invoke automatically when `Bash(run_in_background: true)` or a subagent finishes, rather than layering a `/loop` on top of harness-tracked work — polling in that case is wasted
- Keep `/loop` and `/schedule` distinct: `/loop` is session-scoped (lives in this conversation; dies on new session, restored on `--resume`/`--continue` within 7 days), while `/schedule` creates Anthropic-cloud Routines that run independently of any session and require a Pro/Max/Team/Enterprise plan
- Confirm provider support before relying on them: `ScheduleWakeup`, `Monitor`, and `/schedule` are unavailable on Bedrock/Vertex AI/Foundry, where a bare-prompt `/loop` falls back to a fixed 10-minute schedule
